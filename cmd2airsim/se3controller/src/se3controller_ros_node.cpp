...
void SE3ControllerNode::setControllerParams()
{
    ...
    controller_.setProportionParams(Kp_x, Kp_y, Kp_z, Kv_x, Kv_y, Kv_z, attctrl_tau_,norm_thrust_const_);
...
void SE3ControllerNode::mainLoop(const ros::TimerEvent &event)
{
    ...
    if (!target_received_ || time_since_last_target.toSec() > delay_time_) {
        // 设置默认悬停目标
        control_target_.position << fcu_position_.pose.position.x, fcu_position_.pose.position.y, 8;
